...
# New feature: allow choosing who goes first
starting_player = ""
while starting_player not in ["c", "u"]:
    starting_player = input("Who goes first? Computer (c) or You (u)? ").lower()
    if starting_player not in ["c", "u"]:
        print("Please enter 'c' for computer or 'u' for you.")
current_turn = 'comp' if starting_player == 'c' else 'user'

while True:
    # 1. Check for game over at top of loop
    if (user_hands[0] == 0 and user_hands[1] == 0):
        print("\n--- Game Over ---")
    ...
    if current_turn == 'user':
        ...
        while not player_move_valid:
            play_choice = ""
            # allow resign and draw offers
            while play_choice not in ["pass", "stay", "resign", "draw"]:
                play_choice = input("Pass, Stay, Draw, or Resign? ").lower()
                if play_choice not in ["pass", "stay", "resign", "draw"]:
                    print("Invalid choice. Please enter 'Pass', 'Stay', 'Draw', or 'Resign'.")

            if play_choice == "resign":
                print("You resigned. Computer wins.")
                player_move_valid = True
                game_result = 'resign'
                break

            if play_choice == "draw":
                # Offer draw to AI; AI decides based on position evaluation
                best_move, evaluation = find_best_move(user_hands, comp_hands, search_depth)
                # Decision rule: if AI's evaluation is <= 0 it is not winning and will accept a draw
                if evaluation <= 0:
                    print("Computer accepts the draw. Game ends in a draw.")
                    game_result = 'draw'
                    player_move_valid = True
                    break
                else:
                    print("Computer declines the draw offer.")
                    # Offering a draw consumes the player's turn; switch to AI
                    player_move_valid = True
                    break
        ...
        # Handle resign/draw end states
        if 'game_result' in locals() and game_result == 'resign':
            print("\n--- Game Over ---")
            print("Computer Wins by resignation.")
            break
        if 'game_result' in locals() and game_result == 'draw':
            print("\n--- Game Over ---")
            print("Game ended in a draw by agreement.")
            break

        # After user's turn, switch to computer
        current_turn = 'comp'

    else:
        # --- AI's Turn ---
        print("\n--- Computer's Turn ---")
        # Re-check for AI win/user win before AI's move
        if user_hands[0] == 0 and user_hands[1] == 0:
            print("Computer Wins! Your hands are both 0.")
            break
        if comp_hands[0] == 0 and comp_hands[1] == 0:
        ...
        # After AI's turn, switch to user
        current_turn = 'user'
